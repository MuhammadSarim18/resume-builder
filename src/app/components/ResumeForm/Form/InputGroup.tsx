...
export const INPUT_CLASS_NAME =
  "mt-1  block w-full rounded-md border-gray-300 text-gray-900 outline-none font-normal text-base";
  ...
  return (
    <InputGroupWrapper label={label}>
      <input
        ...
        className={`${className} text-${settings.themeColor}` }
      />
    </InputGroupWrapper>
  );
};
...
export const Textarea = <T extends string>({
  ...
  placeholder,
  onChange,
  className, // New prop to accept additional classes
}: InputProps<T, string>) => {
  ...
  return (
    <InputGroupWrapper label={label} className={wrapperClassName}>
      <textarea
        ref={textareaRef}
        name={name}
        className={`${INPUT_CLASS_NAME} resize-none overflow-hidden ${className}`}
        placeholder={placeholder}
        value={value}
        onChange={(e) => onChange(name, e.target.value)}
      />
