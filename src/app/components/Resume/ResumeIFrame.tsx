...
<html>
  ...
  <body style='overflow: hidden; width: 90%; margin: 10px; padding: 10px; -webkit-text-size-adjust:none;'>
    <div></div>
  </body>
</html>`;
};
...
const getIframeInitialContent2 = (isA4: boolean) => {
  const width = isA4 ? A4_WIDTH_PT : LETTER_WIDTH_PT;
  const allFontFamilies = getAllFontFamiliesToLoad();
...
const ResumeIframe2 = ({
  ...
  const isA4 = documentSize === "A4";
  const iframeInitialContent = useMemo(
    () => getIframeInitialContent2(isA4),
    [isA4]
  );

  if (enablePDFViewer) {
    return (
      <DynamicPDFViewer2 className="h-full w-full ">
        {children as any}
      </DynamicPDFViewer2>
    );
  }
...
export const ResumeIframeCSR2 = dynamic(() => Promise.resolve(ResumeIframe2), {
  ssr: false,
});

// PDFViewer is only used for debugging. Its size is quite large, so we make it dynamic import
const DynamicPDFViewer2 = dynamic(
  () => import("@react-pdf/renderer").then((module) => module.PDFViewer),
  {
    ssr: false,
  }
