...
import cssReset from "@chakra-ui/css-reset/dist/css-reset";
...
function file2Text(file: File): Promise<string> {
...
async function exportAsPng(resume: Resume) {
  try {
    const element = document.getElementById(resume.design.template);
    ...
    // Ensure that i18n is properly initialized before using it
    if (i18n) {
      toast({
        description: i18n.t("something_went_wrong").toString(),
        status: "error",
...
function pt2px(pt: number, suffix = true): string | number {
  if (suffix) {
    return `${(pt * 96) / 72}px`;
  }
  return (pt * 96) / 72;
...
function countWords(summary: string, section: SectionField[]) {
  const summaryCount = isEmpty(summary) ? 0 : length(split(" ", summary));
  const standardSections = filter((item) => item && !isNil(item.nested), section); // Type guard here
  const nested = flatten(
    map((item) => (item ? item.nested : []), standardSections) // Type guard here
  );
  const descriptionCount = map(
    (item) =>
      item && !isEmpty(item.description) // Type guard here
        ? length(split(" ", item.description))
        : 0,
    nested
  );
  return sum([summaryCount, ...descriptionCount]);
